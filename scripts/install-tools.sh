...
if has_group runtime; then "$(dirname "$0")/install-runtime-tools.sh"; fi
...
if has_group vuln; then install_apt nikto masscan whatweb; "$(dirname "$0")/install-runtime-tools.sh"; nuclei -update-templates || true; fi
