...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const local = await db.prepare("SELECT id, program_id FROM jobs WHERE id = ? AND owner_id = ?").bind(id, actor).first<{ id: string; program_id: string }>();
    ...
    const statements: D1PreparedStatement[] = [
      db.prepare("UPDATE jobs SET status = ?, exit_code = ?, error = ?, started_at = ?, finished_at = ?, updated_at = ? WHERE id = ?")
    ...
    for (const result of (job.results || []).slice(0, 10_000)) {
      ...
      statements.push(db.prepare("INSERT OR IGNORE INTO job_results (id, job_id, program_id, result_type, value, normalized_json, searchable_text, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)")
      ...
      if (["host", "url", "service"].includes(result.type)) {
        statements.push(db.prepare("INSERT INTO assets (id, program_id, first_seen_job_id, type, value, metadata_json, last_seen_at, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?) ON CONFLICT(program_id, type, value) DO UPDATE SET metadata_json = excluded.metadata_json, last_seen_at = excluded.last_seen_at, updated_at = excluded.updated_at")
      ...
      if (result.type === "finding-candidate") {
        ...
        statements.push(db.prepare("INSERT OR IGNORE INTO findings (id, program_id, source_job_id, title, vulnerability_class, severity, status, confidence, summary, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, 'candidate', 60, ?, ?, ?)")
    ...
    if (job.stdout) {
      ...
      const candidate = await db.prepare("SELECT id FROM findings WHERE source_job_id = ? ORDER BY created_at LIMIT 1").bind(id).first<{ id: string }>();
      if (candidate) await db.prepare("INSERT OR IGNORE INTO evidence (id, finding_id, job_id, kind, content, sha256, redacted, created_by, created_at) VALUES (?, ?, ?, 'stdout', ?, ?, 1, ?, ?)")
