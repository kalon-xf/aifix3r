...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const job = await db.prepare("SELECT id FROM jobs WHERE id = ? AND owner_id = ?").bind(id, actor).first();
    if (!job) return Response.json({ error: "Job not found." }, { status: 404 });
    const response = await callWorker(`/v1/jobs/${encodeURIComponent(id)}/cancel`, { method: "POST", body: "{}" });
    const payload = await response.json();
    if (response.ok) await db.prepare("UPDATE jobs SET status = 'cancelled', updated_at = ? WHERE id = ?").bind(Date.now(), id).run();
