...
export async function GET(request: Request) {
  ...
  try {
    const sql = programId ? "SELECT * FROM jobs WHERE owner_id = ? AND program_id = ? ORDER BY created_at DESC LIMIT 200" : "SELECT * FROM jobs WHERE owner_id = ? ORDER BY created_at DESC LIMIT 200";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const program = await db.prepare("SELECT * FROM programs WHERE id = ? AND owner_id = ? AND status IN ('draft', 'active')").bind(input.programId, actor).first<Record<string, unknown>>();
    if (!program) return Response.json({ error: "Authorized program not found or paused." }, { status: 404 });
    const scopes = (await db.prepare("SELECT value, decision FROM scopes WHERE program_id = ?").bind(input.programId).all<{ value: string; decision: string }>()).results;
    ...
    await db.prepare("INSERT INTO jobs (id, program_id, owner_id, tool, target, parameters_json, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)")
