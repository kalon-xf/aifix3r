...
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const program = await db.prepare("SELECT id FROM programs WHERE id = ? AND owner_id = ?").bind(id, actor).first();
    if (!program) return Response.json({ error: "Program not found." }, { status: 404 });
    const result = await db.prepare("SELECT * FROM scopes WHERE program_id = ? ORDER BY decision, value").bind(id).all();
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const db = await getD1();
    const program = await db.prepare("SELECT id FROM programs WHERE id = ? AND owner_id = ?").bind(programId, actor).first();
    if (!program) return Response.json({ error: "Program not found." }, { status: 404 });
    const scope = { id: crypto.randomUUID(), programId, kind, value, decision, notes: input.notes?.slice(0, 500) || null, createdAt: Date.now() };
    await db.prepare("INSERT INTO scopes (id, program_id, kind, value, decision, notes, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)")
