...
export async function GET(request: Request) {
  ...
  try {
    ...
    const sql = `SELECT r.* FROM job_results r JOIN programs p ON p.id = r.program_id WHERE p.owner_id = ? AND (? IS NULL OR r.program_id = ?) AND (? = '' OR r.searchable_text LIKE ? ESCAPE '\\') ORDER BY r.created_at DESC LIMIT 500`;
