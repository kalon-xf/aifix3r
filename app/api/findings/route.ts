...
export async function GET(request: Request) {
  ...
  try {
    const results = await (await getD1()).prepare(`SELECT f.* FROM afx_findings f JOIN afx_programs p ON p.id = f.program_id WHERE p.owner_id = ? AND (? IS NULL OR f.program_id = ?) AND (? IS NULL OR f.status = ?) ORDER BY f.updated_at DESC LIMIT 200`)
