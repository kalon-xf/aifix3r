...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    await db.batch([
      db.prepare("INSERT INTO finding_validations (id, finding_id, reviewer_id, decision, rationale, created_at) VALUES (?, ?, ?, ?, ?, ?)").bind(crypto.randomUUID(), id, actor, decision, rationale, now),
      db.prepare("UPDATE findings SET status = ?, validator_id = ?, validated_at = ?, updated_at = ? WHERE id = ?").bind(status, actor, decision === "approve" ? now : null, now, id),
    ]);
