...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const finding = await db.prepare("SELECT f.* FROM afx_findings f JOIN afx_programs p ON p.id = f.program_id WHERE f.id = ? AND p.owner_id = ? AND f.status IN ('validated', 'reported', 'resolved')").bind(id, actor).first<Record<string, string | null>>();
    if (!finding) return Response.json({ error: "Only a human-validated finding can be reported." }, { status: 409 });
    const evidence = (await db.prepare("SELECT kind, content FROM afx_evidence WHERE finding_id = ? ORDER BY created_at").bind(id).all<{ kind: string; content: string }>()).results;
    ...
    await db.batch([
      db.prepare("INSERT INTO afx_reports (id, finding_id, platform, format_version, content, created_by, created_at) VALUES (?, ?, ?, 1, ?, ?, ?)").bind(reportId, id, platform, content, actor, now),
      db.prepare("UPDATE afx_findings SET status = 'reported', updated_at = ? WHERE id = ?").bind(now, id),
    ]);
